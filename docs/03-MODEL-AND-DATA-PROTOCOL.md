...
The repository now contains a decoded March 2023 sample from five MYD14A1 composites and 31 VNP14A1 daily granules. The pipeline verifies full calendar coverage, the shared h26v06 sinusoidal grid, QA land states, projected pixel-center AOI clipping, and 10×10 native-pixel blocks. It emits native-only artifacts with source-object SHA-256 checksums. The two current bounding boxes are candidate windows, not frozen Curated Regions; local file retrieval timestamps were not retained.

This sample does not establish a transfer model, temporal stability, geographic transfer, interval coverage, independent fire truth, or impact on a decision. It is a processing proof, not a comparative result. The old root-level `build_timeseries.py` inventory is full-tile exploratory output and must not be used as an AOI-clipped result or calibration input. An observed hotspot is not independently verified wildfire truth.
